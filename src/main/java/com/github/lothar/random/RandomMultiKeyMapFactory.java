    ...
    public MultiKeyMap create() {
        ...
        for (int i = 0; i < elementsCount; i++) {
            if (getGenericTypes().length == 1) {
                map.put(RandomFactoryToolkit.generate(getGenericTypes()[0]), RandomFactoryToolkit.generate(valueType));
                ...
                map.put(RandomFactoryToolkit.generate(getGenericTypes()[0]), RandomFactoryToolkit
                                .generate(getGenericTypes()[1]), RandomFactoryToolkit.generate(getGenericTypes()[2]),
                                RandomFactoryToolkit.generate(valueType));
            } else if (getGenericTypes().length == 4) {
                map.put(RandomFactoryToolkit.generate(getGenericTypes()[0]), RandomFactoryToolkit
                                .generate(getGenericTypes()[1]), RandomFactoryToolkit.generate(getGenericTypes()[2]),
                                RandomFactoryToolkit.generate(getGenericTypes()[3]), RandomFactoryToolkit
                                                .generate(valueType));
